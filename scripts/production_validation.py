        ...
        try:
            ...
            if mode == "dashboard":
                ...
                entry = {
                    ...
                    **{key: decision.get(key, 0.0) for key in _SCORE_KEYS},
                    ...
                    "research_status": result.get("research_summary", {}).get("status", "unknown"),
                    "research_data_quality": result.get("research_data_quality", "unknown"),
                    "mvp_status": result.get("mvp_plan", {}).get("status", "unknown"),
                    "build_status": result.get("build_status", {}).get("status", "unknown"),
                    "deployment_status": result.get("deployment_status", {}).get("status", "unknown"),
                    "growth_status": result.get("growth_status", {}).get("status", "unknown"),
                ...
                entry = {
                    ...
                    **{key: result.get(key, 0.0) for key in _SCORE_KEYS},
                    ...
                    "status": result.get("status", "unknown"),
                    "research_status": "not run (decision mode)",
                    "research_data_quality": "not available (decision mode)",
                    "mvp_status": "not run (decision mode)",
                    "build_status": "not run (decision mode)",
                    "deployment_status": "not run (decision mode)",
                    "growth_status": "not run (decision mode)",
            ...
            entry = {
                ...
                **{key: 0.0 for key in _SCORE_KEYS}, "overall_score": 0.0, "recommendation": "ERROR",
                "status": "exception",
                "research_status": "unknown", "research_data_quality": "unknown", "mvp_status": "unknown", "build_status": "unknown",
                "deployment_status": "unknown", "growth_status": "unknown",
                "top_reason": "", "top_warning": "",
...
def write_idea_scorecard_csv(scorecard: list[dict[str, Any]], path: Path) -> None:
    fieldnames = [
        ...
        "research_status", "research_data_quality", "mvp_status", "build_status", "deployment_status", "growth_status",
...
def write_idea_scorecard_html(scorecard: list[dict[str, Any]], path: Path) -> None:
    ...
    for i, e in enumerate(scorecard):
        ...
        row_parts.append(
            ...
            f"<td>{esc(e.get('research_status', 'n/a'))}</td>"
            f"<td>{esc(e.get('research_data_quality', 'n/a'))}</td>"
            f"<td>{esc(e.get('mvp_status', 'n/a'))}</td>"
            f"<td>{esc(e.get('build_status', 'n/a'))}</td>"
            f"<td>{esc(e.get('deployment_status', 'n/a'))}</td>"
            f"<td>{esc(e.get('growth_status', 'n/a'))}</td>"
            f"<td>{esc(e['top_reason'] or e['error'])}</td><td>{e['execution_time_seconds']:.1f}s</td></tr>"
        )
    rows = "".join(row_parts)
...
<main><table>
<tr><th>#</th><th>Idea</th><th>Overall Score</th><th>Recommendation</th><th>Research</th><th>Data Quality</th><th>MVP</th><th>Build</th><th>Deploy</th><th>Growth</th><th>Top Reason / Error</th><th>Time</th></tr>
{rows}
</table></main>
